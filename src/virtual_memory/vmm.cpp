...
VirtualMemoryManager::VirtualMemoryManager(Size physical_memory_size, Size page_size, PageReplacementPolicy policy)
...
bool VirtualMemoryManager::deallocatePages(ProcessId process_id, size_t num_pages) {
    ...
    for (const auto& entry : entries) {
        if (deallocated >= num_pages) break;
        if (entry.second.present) {
            freeFrame(entry.second.frame_number);
...
VirtualMemoryManager::VMMStats VirtualMemoryManager::getStats() const {
    ...
    stats.page_fault_rate = total_accesses_ > 0
        ? static_cast<double>(page_faults_) / total_accesses_
        : 0.0;

    stats.free_frames = 0;
    for (bool allocated : frame_allocation_) {
        if (!allocated) stats.free_frames++;
    }
